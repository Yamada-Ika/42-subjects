...
GRANT ALL ON ${WORDPRESS_DB_NAME}.* to '${MARIADB_USER}'@'%' IDENTIFIED BY '${MARIADB_USER_PASS}';
GRANT ALL ON ${WORDPRESS_DB_NAME}.* to '${MARIADB_USER}'@'localhost' IDENTIFIED BY '${MARIADB_USER_PASS}';
FLUSH PRIVILEGES;
EOF
